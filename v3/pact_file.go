...
// PactFileBody is what will be serialised to the Pactfile in the request body examples and matching rules
...
func PactBodyBuilder(root map[string]interface{}) PactFileBody {
	pactFile := PactFileBody{}
	fmt.Printf("root: %+v", root)
	_, pactFile.Body, pactFile.MatchingRules = build("", root, make(map[string]interface{}),
