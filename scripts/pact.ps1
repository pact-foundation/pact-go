...
if !(Test-Path -Path $pactBinariesPath) {
...
# $packages = go list -buildvcs=false github.com/pact-foundation/pact-go/... |  where {$_ -inotmatch 'vendor'} | where {$_ -inotmatch 'examples'}
$curDir=$pwd
